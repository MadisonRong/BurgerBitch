  ...
  scope :get_dishes, ->(restaurant_id){
    ...
    ***REMOVED*** 分类展示菜式
    ...
    while iterator < dishes_size
      if iterator ==0
        ***REMOVED*** logger.info "the first one is: ***REMOVED***{dishes[iterator].name}***REMOVED******REMOVED***{dishes[iterator].dish_type.name}"
        ***REMOVED*** 第一个元素，直接入数组
      ...
      if iterator + 1 == dishes_size
        ***REMOVED*** logger.info "the last one is: ***REMOVED***{dishes[iterator].name}***REMOVED******REMOVED***{dishes[iterator].dish_type.name}"
        ...
        ***REMOVED*** if dishes[iterator].dish_type.name != dishes[iterator - 1].dish_type.name
        ***REMOVED***   ***REMOVED*** 不相同，将前面积累的数组赋值给对应的hash
        ***REMOVED***   if result_hash[dishes[iterator - 1].dish_type.name.to_s].nil?
        ***REMOVED***     result_hash[dishes[iterator - 1].dish_type.name.to_s] = result_array
        ***REMOVED***   else
        ***REMOVED***     result_hash[dishes[iterator - 1].dish_type.name.to_s] += result_array
        ***REMOVED***   end
        ***REMOVED***   ***REMOVED*** 开辟新数组储存接下来的元素
        ***REMOVED***   result_array = Array.new
        ***REMOVED***   ***REMOVED*** 将当前值加入数组
        ***REMOVED***   result_array << dishes[iterator]
        ***REMOVED***   result_hash[dishes[iterator].dish_type.name.to_s] = result_array
        ***REMOVED*** else
        ***REMOVED***   ***REMOVED*** 相同则直接加入数组，并且将数组赋值给hash
        ***REMOVED***   result_array << dishes[iterator]
        ***REMOVED***   result_hash[dishes[iterator - 1].dish_type.name.to_s] = result_array
        ***REMOVED*** end
        iterator += 1
      ...
      ***REMOVED*** logger.info "***REMOVED***{dishes[iterator].name}***REMOVED******REMOVED***{dishes[iterator].dish_type.name}"
      ***REMOVED*** 判断当前菜式的类型是否与前一个不相同
